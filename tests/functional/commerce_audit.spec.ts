...
test.group('commerce audit — products', (group) => {
  ...
  test('a stock movement is audited on top of the stock ledger', async ({ client, assert }) => {
    ...
    assert.equal(Number(logs[0].performed_by), worker.id)
  })
...
test.group('commerce audit — sales', (group) => {
  ...
  test('cancelling a sale is audited, and names who cancelled it', async ({ client, assert }) => {
    ...
    assert.equal(Number(cancelLog!.performed_by), admin.id)
    assert.notEqual(Number(cancelLog!.performed_by), worker.id)
  })
...
test.group('commerce audit — the read endpoint', (group) => {
  ...
  test('worker cannot read it — no audit permission', async ({ client }) => {
    const worker = await createWorker()
    const response = await client.get('/api/v1/audit/commerce').loginAs(worker)
    response.assertStatus(403)
  })

  test('customer cannot read it', async ({ client }) => {
    const customer = await createCustomer()
    const response = await client.get('/api/v1/audit/commerce').loginAs(customer)
    response.assertStatus(403)
  })
})
